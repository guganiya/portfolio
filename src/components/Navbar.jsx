...
const Navbar = () => {
  ...
  // Separate refs to ensure click-outside works flawlessly on both mobile and desktop views
  ...
  // Handles clicking outside of dropdown menus to close them automatically
  useEffect(() => {
  ...
  // Locks screen scrolling behind open mobile overlay to prevent jarring layout behavior
  useEffect(() => {
    ...
    <nav className="fixed w-full z-[100] bg-[#0a0a0a]/90 backdrop-blur-xl border-b border-white/5">
      <div className="max-w-7xl mx-auto px-6 lg:px-10">
        <div className="flex items-center justify-between h-24">
          ...
          <div className="flex-shrink-0 z-[110]">
            ...
            >
              <span className="text-3xl font-black tracking-tighter bg-gradient-to-r from-white via-white to-[#d4af37] bg-clip-text text-transparent cursor-pointer uppercase select-none">
                {t("common.logo")}
              </span>
            </RouterLink>
          </div>
          ...
          <div className="flex items-center gap-4 md:gap-6 z-[110]">
            ...
            <div className="md:hidden relative" ref={mobileLangRef}>
              <button
                onClick={() => setLangOpen(!langOpen)}
                className="flex items-center gap-1.5 px-3 py-2 min-h-[40px] rounded-lg bg-white/5 border border-white/10 text-xs font-bold text-zinc-300 uppercase active:bg-white/10 transition-colors touch-none select-none"
              >
                {currentLang}
                <motion.div
                  animate={{ rotate: langOpen ? 180 : 0 }}
              ...
              <AnimatePresence>
                {langOpen && (
                  <motion.div
                    ...
                    exit={{ opacity: 0, y: 8 }}
                    transition={{ duration: 0.15 }}
                    className="absolute right-0 mt-2 w-28 py-1.5 bg-[#1a1a1a] border border-white/10 rounded-lg shadow-2xl backdrop-blur-xl"
                  >
                    {languages.map((lang) => (
                      <button
                        key={lang.code}
                        onClick={() => changeLanguage(lang.code)}
                        className={`w-full text-left px-4 py-2.5 text-xs font-bold transition-colors ${currentLang === lang.code ? "text-[#d4af37]" : "text-zinc-400 active:text-white"}`}
                      >
                        {lang.label}
                      </button>
                    ))}
            ...
            {/* BURGER BUTTON - Sized properly for mobile touch targets */}
            <button
              onClick={() => setIsOpen(!isOpen)}
              aria-label="Toggle Menu"
              className="md:hidden flex flex-col items-end justify-center w-11 h-11 p-2 rounded-lg bg-white/5 border border-white/10 space-y-1.5 focus:outline-none select-none active:bg-white/10 transition-colors"
            >
              <motion.span
                animate={
                  isOpen
      ...
      <AnimatePresence>
        {isOpen && (
          ...
          >
            <div className="flex flex-col items-center space-y-8 w-full max-w-xs px-6">
              {navLinks.map((link, i) => {
                // Excludes "Projects" on mobile devices per your specifications
                ...
                return (
                  ...
                  >
                    <RouterLink
                      to={link.type === "scroll" ? `/#${link.to}` : link.to}
                      onClick={() => setIsOpen(false)}
                      className="block text-4xl font-black text-zinc-500 active:text-[#d4af37] hover:text-[#d4af37] cursor-pointer uppercase transition-colors py-2 select-none tracking-tight"
                    >
                      {link.name}
                    </RouterLink>
                  </motion.div>
              ...
              {/* MOBILE SOCIALS */}
              <div className="flex gap-8 pt-8 border-t border-white/5 w-full justify-center">
                {socialLinks.map((s) => (
                  <a
                    key={s.name}
                    href={s.href}
                    className="text-zinc-500 active:text-white p-2 transition-colors"
                    target="_blank"
                    rel="noreferrer"
                  >
                    {s.icon}
